...
const ProductCard = ({ product }:{ product: Product }) => {
  ...
  return (
    <div className=" text-sm border-[1px] border-dark-blue/50 rounde-md bg-white group">
      ...
      {product?.status === "new" && ( 
        <p className="absolute top-2 left-2 z-10 text-sm border 
    ...
    {product?.status === "hot" && ( 
        <Link
        href={"/deal"} className="absolute top-2 left-2 z-10 text-sm border 
  ...
  {product?.categories && (
    <p className="uppercase text-xs line-clamp-1 text-shop-light-text">{product.categories.map((cat) => cat).join(", ")}</p>
  )}
  ...
  <div className="flex items-center gap-2.5">
      ...
      <p className={`${product?.stock === 0 ? "text-red-600" :
         "text-shop-light-green/80 font-semibold"}`
          ...
          {(product?.stock as number) > 0 ?product?.stock : "Out of Stock"}</p>
