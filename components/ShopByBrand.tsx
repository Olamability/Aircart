    ...
    <div className="mb-10 lg:pb-10 bg-shop-light-bg p-5 lg:p-7 rounded-md ">
     ...
     <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 mt-16 border-2 border-shop-light-green/30 bg-shop-light-green/20 p-2 shadow-sm inner hover:shadow-shop-light-green/50 py-5">
      {serviceData?.map((item, index) => (
        <div key={index} className="flex items-center gap-3 group text-lightColor">
          <span className="inline-flex scale-100 group-hover:scale-90 hover:text-shop-light-green hoverEffect">{item?.icon}</span>
          <div className="text-sm"><p className="text-darkColor/80 font-bold capitalize">{item?.title}</p><p className="text-lightColor">{item?.description}</p></div>
